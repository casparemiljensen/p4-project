...
public class BuildASTVisitor extends eelBaseVisitor<AbstractNode> implements eelVisitor<AbstractNode> {
	...
	public ProcedureNode visitProcedure(eelParser.ProcedureContext ctx) {
		...
		return new ProcedureNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(), ctx.ID(),
				((ctx.formalParams() != null) ? visitFormalParams(ctx.formalParams()) : null), statementNodes);
	}
	...
	public DeclarationNode visitDeclaration(eelParser.DeclarationContext ctx) {
		return new DeclarationNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(), "let", ctx.ID(),
				visitAssignment(ctx.assignment()));
	}

	@Override
	public AssignmentNode visitAssignment(eelParser.AssignmentContext ctx) {
		return new AssignmentNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(), "assign",
		...
		// expr op expr
			...
			if (ctx.operator().binaryOperator() != null) {
				...
				switch (ctx.operator().binaryOperator().BINARYOP().getText()) {
					case "+":
					case "-": {
						node = new AddSubNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().binaryOperator().BINARYOP(),
								visitExpression(ctx.right));
						break;
					}
					case "*":
					case "/": {
						node = new MultDivNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().binaryOperator().BINARYOP(),
								visitExpression(ctx.right));
						break;
					}
				...
				switch (ctx.operator().booleanOperator().BOOLEANOP().getText()) {
					case "<":
						node = new LessThanNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().booleanOperator().BOOLEANOP(),
								visitExpression(ctx.right));
						break;
					case ">":
						node = new GreaterThanNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().booleanOperator().BOOLEANOP(),
								visitExpression(ctx.right));
						break;
					case "<=":
						...
						break;
					case ">=":
						node = new GreaterThanOrEqualNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().booleanOperator().BOOLEANOP(),
								visitExpression(ctx.right));
						break;
					case "==":
						node = new EqualsNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().booleanOperator().BOOLEANOP(),
								visitExpression(ctx.right));
						break;
					case "!=":
						node = new NotEqualNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
								visitExpression(ctx.left), ctx.operator().booleanOperator().BOOLEANOP(),
								visitExpression(ctx.right));
						break;
					default:
				...
				return node;
				// return new ExpressionNode(ctx.start.getLine(),
				// ctx.start.getCharPositionInLine(), visitExpression(ctx.left),
				// visitOperator(ctx.operator()), visitExpression(ctx.right));
				// '-' expr
			}
	...
	public ValueNode visitValue(eelParser.ValueContext ctx) {
		...
		if (ctx.userValue() == null)
			return new ValueNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
					visitStaticValue(ctx.staticValue()));
		// userValue
		else
			return new ValueNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
	...
	public UserValueNode visitUserValue(eelParser.UserValueContext ctx) {
		return new UserValueNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(), ctx.ID(),
	...
	public StaticValueNode visitStaticValue(eelParser.StaticValueContext ctx) {
		...
		// function
			...
			return new StaticValueNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
					visitFunction(ctx.function()), ((ctx.method() != null) ? visitMethod(ctx.method()) : null));
	}
	...
	public ControlStructNode visitControlStruct(eelParser.ControlStructContext ctx) {
		...
		if (ctx.iterativeStruct() == null)
			return new ControlStructNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
					visitSelectiveStruct(ctx.selectiveStruct()));
		// iterativeStruct
		else
			return new ControlStructNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
					visitIterativeStruct(ctx.iterativeStruct()));
	}

	@Override
	public SelectiveStructNode visitSelectiveStruct(eelParser.SelectiveStructContext ctx) {
		return new SelectiveStructNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
				visitIfStruct(ctx.ifStruct()));
	}

	@Override
	public ReturnNode visitReturn(eelParser.ReturnContext ctx) {
		return new ReturnNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(), "return",
				visitExpression(ctx.expression()));
	}
	...
	public IfStructNode visitIfStruct(eelParser.IfStructContext ctx) {
		List<ParseTree> input2 = ctx.children.stream().filter(e -> e instanceof eelParser.StatementContext)
				.collect(Collectors.toList());
		List<StatementNode> statements = CreateList(input2, StatementNode.class);
		List<ParseTree> input1 = ctx.children.stream().filter(e -> e instanceof eelParser.ElseIfStructContext)
				.collect(Collectors.toList());
		List<ElseIfStructNode> ElseIfStructNodes = CreateList(input1, ElseIfStructNode.class);
		return new IfStructNode(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
				visitIfCondition(ctx.ifCondition()), "then", statements, ElseIfStructNodes,
				(ctx.elseStruct() != null) ? visitElseStruct(ctx.elseStruct()) : null);
	}
